...
#define ECS_QD_CHANNEL  3   /* Primary eTPU channel: Use eTPU channel 3 for QD and ECS interface board that corresponds to connector Encoder 2 */
...
void init_FQD()
{
    ...
    error_code = fs_etpu_qd_init(
		...
		0,         			 /* slow_normal_threshold */
		0,            		 /* normal_slow_threshold */
		0,            		 /* normal_fast_threshold */
		0,            		 /* fast_normal_threshold */
		...
		4000);		             /* qd_pc_per_rev (we use a 1000-pulse encoder with 2 channels) */
...
uint16_t ReadFQD_pc()
{
	...
	int24_t input = fs_etpu_qd_get_pc(ECS_QD_CHANNEL);
	
	return  (uint16_t) (input & 0x00FFFF);
}


int32_t updateCounter()
{
	static uint16_t PREV_FQDPC = 0;
	static int32_t LAST_TOTAL = 0;
	static uint16_t CURR_FQDPC = 0;
	CURR_FQDPC = ReadFQD_pc();
	LAST_TOTAL = LAST_TOTAL + (int16_t)(CURR_FQDPC - PREV_FQDPC);
	//LAST_TOTAL = LAST_TOTAL + (CURR_FQDPC - PREV_FQDPC);
...
float updateAngle()
{
  int32_t counts = updateCounter();
  float angle = 0.09*counts;
  return angle; 
}
