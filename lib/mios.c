...
{
    EMIOS.MCR.B.GPRE = prescaler; //CHANGED                    /* Clock divider , divided by 1 --> max. frequency equal to system clock 120 MHz */
...
void init_PWM(int miosChannel, uint32_t newPeriod_ns)
{
    ...
    EMIOS.CH[miosChannel].CCR.B.UCPRE = 0;          /* Select a prescaler that allows you the maximum duty cycle resolution */
    EMIOS.CH[miosChannel].CCR.B.UCPREN = 0b1;         /* Turn on channel prescaler to have internal clock */
    EMIOS.CH[miosChannel].CCR.B.BSL = 0b11;            /* Must use internal counter in this mode */
    EMIOS.CH[miosChannel].CCR.B.EDPOL = 0b0;          /* Match CADR clears output, match CBDR sets output */
    EMIOS.CH[miosChannel].CCR.B.MODE = 0b1011010;           /* OPWFMB Output Pulse Width and Frequency Modulation, Buffered Mode
    ...
    SIU.PCR[179 + miosChannel].B.PA = 0b1;
...
void set_PWMPeriod_ns(int miosChannel, uint32_t newPeriod_ns)
{
    ...
    */
	...
	EMIOS.CH[miosChannel].CBDR.R = ticks;
                                             /* set PWM period (ticks)*/
...
int8_t set_PWMDutyCycleLimits(int miosChannel, float minimum, float maximum) // Input to this should be 0.24 and 0.76)
{
...
void set_PWMDutyCycle(int miosChannel, float dutyCycle)
{
	...
	//int8_t status = set_PWMDutyCycleLimits(miosChannel, 0.24, 0.76);
	
	if(dutyCycle < DutyCycleLimits[MINIMUM]) dutyCycle = DutyCycleLimits[MINIMUM];
	if(dutyCycle > DutyCycleLimits[MAXIMUM]) dutyCycle = DutyCycleLimits[MAXIMUM];

	
	if(dutyCycle >= DutyCycleLimits[MINIMUM] || dutyCycle <= DutyCycleLimits[MAXIMUM]){
		DutyCycle[miosChannel] = dutyCycle;
		/* Write value to eMIOS register here */
		EMIOS.CH[miosChannel].CADR.R = EMIOS.CH[miosChannel].CBDR.R * dutyCycle;
		//debug = EMIOS.CH[miosChannel].CADR.R;
	}
    return;
}
